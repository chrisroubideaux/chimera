...
const dailyAverageSales = 11.4;
const currentWeekSales = generateWeeklySalesData(dailyAverageSales);
const previousWeekSales = generateWeeklySalesData(dailyAverageSales * 0.96);
const averageSales = Array(labels.length).fill(dailyAverageSales);
...
export default function WeeklyChart() {
  ...
  const generateCSV = () => {
    ...
    const rows = labels.map((label, index) => [
      label,
      currentWeekSales[index] ? `${currentWeekSales[index]}K` : '',
      previousWeekSales[index] ? `${previousWeekSales[index]}K` : '',
      averageSales[index] ? `${averageSales[index]}K` : '',
    ]);
    ...
    link.href = url;
    link.setAttribute('download', `weekly_sales_${currentWeek}.csv`);
