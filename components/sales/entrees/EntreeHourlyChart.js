...
export default function EntreeHourlyChart({ setActiveComponent }) {
  ...
  const generateCSV = () => {
    const rows = [
      ['Hour', 'Actual Sales (k)', 'Projected Sales (k)', 'Average Sales (k)'],
    ];

    labels.forEach((label, index) => {
      const actual = chartData.datasets[0].data[index];
      const projected = chartData.datasets[1].data[index];
      const average = chartData.datasets[2].data[index];
      rows.push([label, actual, projected, average]);
    });
