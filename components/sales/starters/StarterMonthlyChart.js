...
export default function StarterMonthlyChart({ setActiveComponent }) {
  ...
  const generateCSV = () => {
    ...
    const rows = chartData.labels.map((month, index) => [
      month,
      `${(chartData.projectedSalesData[index] / 1000).toFixed(0)}k`, // Projected Sales
      `${(chartData.actualSalesData[index] / 1000).toFixed(0)}k`, // Actual Sales
      `${(chartData.averageSales[index] / 1000).toFixed(0)}k`, // Average Sales
    ]);
