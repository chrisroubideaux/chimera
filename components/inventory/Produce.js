...
export default function Produce({ produce }) {
  ...
  // Function to generate sales data with more realism
  ...
  useEffect(() => {
    // Update the sales data with generated values
...
export default function Produce({ produce }) {
  ...
  const generateSalesData = (data) => {
    return data.map((item) => {
      ...
      // Generating a random number of sold items
      const sold = faker.datatype.number({ min: 0, max: count });
