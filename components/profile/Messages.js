...
export default function Messages({ setActiveComponent }) {
  ...
  const [sender] = useState('66d7d2c380470662dbca3239'); // Employee sender ID
  const [recipient] = useState('66feb2d7bfdb4d747e58bcb9'); // Admin recipient ID
...
export default function EmployeeMessages({ setActiveComponent }) {
  const [newMessage, setNewMessage] = useState('');
  const [sender] = useState('66d7d2c380470662dbca3239'); // Employee sender ID
  const [recipient] = useState('66feb2d7bfdb4d747e58bcb9'); // Admin recipient ID
  const [messages, setMessages] = useState([]);
              ...
              {messages.length === 0 ? (
                ...
                messages.map((message) => (
                  <div
                    key={message._id}
                    className={`d-flex mb-1 ${
                      message.sender._id === sender ? 'justify-content-end' : ''
                    }`}
                  >
                    <div
                      className={`flex-shrink-0 avatar avatar-xs me-2 ${
