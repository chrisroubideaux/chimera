...
export default function Schedule({ setActiveComponent, meetings }) {
  const today = format(new Date(), ' MM/dd/yyyy');
  const dayOfWeek = format(new Date(), 'EEEE');
  return (
    <div>
                  ...
                  <div className="">
                    <div className="list-group" style={{ width: '50rem' }}>
                      ...
                      {meetings && meetings.length > 0 ? (
                        ...
                        <span className="pt-1 form-checked-content" key={meeting._id}>
                          <strong className="d-flex me-5"> {today}</strong>
                          <h5 className="d-block  ">
                          <h5>{meeting.isVideo ? 'Video' : 'In-Person'}</h5>
                            <i className="social-icon fa-solid fa-calendar-days me-1 "></i>
                            {dayOfWeek}: {meeting.time}
                          </h5>
                          <strong className=""> {meeting.sender.name} to {meeting.recipient.name} </strong>
                        </span>
