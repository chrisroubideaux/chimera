...
const Beverages = ({ setActiveComponent, beverages, setSelectedBevereage }) => {
  ...
  const handleEdit = (beverage) => {
    setActiveComponent('EditBev');
    setSelectedBevereage(beverage);
  };

  const handleExportCSV = () => {
    const csvHeaders = ['Name', 'Category', 'Price', 'Count', 'Par', 'Sales'];
    const csvRows = beverages.map((beverage) => [
      beverage.name,
      beverage.category,
      beverage.price,
      beverage.count,
      beverage.par,
      dailySales.find((sale) => sale.id === beverage.id)?.sales.toFixed(2),
    ]);
    ...
    link.setAttribute('download', 'beverages.csv');
