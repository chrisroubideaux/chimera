...
export default function EditForms({ setActiveComponent, selectedStarter }) {
  const [isEditing, setIsEditing] = useState(false);
  const [starter, setStarter] = useState(selectedStarter);

  // Update the starter state if selectedStarter changes
  useEffect(() => {
    if (selectedStarter) {
  ...
  if (!starter || Object.keys(starter).length === 0) {
    return <p>No starter data available.</p>;
  }
  ...
  // Delete Starter/appietizer
  const handleDelete = () => {
    let startersList = JSON.parse(localStorage.getItem('starters')) || [];
    startersList = startersList.filter((item) => item._id !== starter._id);
    localStorage.setItem('starters', JSON.stringify(startersList));
        ...
        <div className="d-grid gap-3 gap-lg-5">
          <div className="card" style={{ maxWidth: '600px' }}>
            <div className="card-header border-bottom">
              <h4 className="card-header-title">Edit {starter.name}</h4>
            </div>
